...
class AppStore extends Component {
  ...
  getFilterItem = SearchResult => {
    const {ActiveItem} = this.state
    const filterItem = SearchResult.filter(
      eachItem => eachItem.category === ActiveItem,
    )
    return filterItem
  }

  render() {
    const {ActiveItem, onChangeInput} = this.state
    const SearchResult = this.getSearchedApps()
    const filterItem = this.getFilterItem(SearchResult)
    return (
      <div className="main-con">
        ...
        <ul className="UList-apps">
          {filterItem.map(eachItem => (
